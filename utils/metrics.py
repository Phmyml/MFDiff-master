...
def per_target_transform(y_pred, y_true):
    ...
    # - `y_pred` must be in the following shape (batch_size, num_categories, ...), float32 possibility
    # - `y_true` must be in the following shape (batch_size, ...) or (batch_size, num_categories, ...), int64.
    assert y_pred.ndim - y_true.ndim in [0, 1]   #.ndim有几个维度
...
def get_evaluation_metric(config, logger):
    """
    ...
    :return: an instance of the evaluation metric    .to(dist_util.dev())
    """
    ...
    metrics_dict = {}
    curves_dict = {}
    cm = _ConfusionMatrix(num_classes=num_classes, threshold=threshold).to(dist_util.dev())
    cm_per_target = None
    cms = None
    cms_per_target = None
    if any([item.startswith('per_target_') for item in metrics]):
        cm_per_target = _ConfusionMatrix(num_classes=num_classes, threshold=threshold).to(dist_util.dev())
    if any(['auc' in item for item in metrics]) or any(['ap' in item for item in metrics]) \
        ...
        if thresholds_type == 'logspace':
            ...
            exit(1)
        cms = [_ConfusionMatrix(num_classes, t, True).to(dist_util.dev()) for t in thresholds]
        cms_per_target = None
        if any([item.startswith('per_target_') for item in curves]) or any(
                item.startswith('per_target_') for item in metrics):
            cms_per_target = [_ConfusionMatrix(num_classes, t, True).to(dist_util.dev()) for t in thresholds]
    ...
    for metric_name in metrics:
        ...
        if metric_name.endswith('tp'):
            metrics_dict[metric_name] = TruePositive(used_cm, update=update_flags[update_flag_id],
                                                     callback_fn=callback_fn).to(dist_util.dev())
            ...
            metrics_dict[metric_name] = HD95(threshold).to(dist_util.dev())
        else:
