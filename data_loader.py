...
class TaskListQueue:
    ...
    def __init__(self, config, subset, logger: logging.Logger, max_num_files=0, shuffle_files=True):
        ...
        if not os.path.exists(instance_filename):
            logger.log('data_list_file does not exist: %s' % instance_filename)
        ...
        if len(instances) == 0:
            logger.log('cannot read instances from %s' % instance_filename)
                ...
                while retry_count < max_retry_num and not is_loaded:
                    try:
                        ...
                        if img_type == 'mask':
                            ...
                            self.logger.log('unrecognized feature or label type: %s' % img_type)
                            dtype = np.int16
                        ...
                        if retry_count < max_retry_num:
                            retry_count += 1
                            self.logger.log('Cannot read from file %s: %s. Retry time %d'
                                                % (os.path.join(self.data_root, ins[file_filed]), e, retry_count))
                            continue
                        else:
                            self.logger.log('Cannot read from file %s: %s. Skip this instance.'
        ...
        while True:
            try:
                ...
                if time.time() - since > 1800:
                    self.logger.log('Failed to get data from data_queue for 1800s and it is not all consumed')
                    return
                time.sleep(2)
                continue
...
class AneurysmSegTestDataset(torch.utils.data.IterableDataset):
    def __init__(self,
        ...
        if config.get('data') is None:
            logger.log('config file does not contain data config')
            exit(1)
        self.logger = logger
        if 'img_spacing' in config['data'] and config['data']['img_spacing'] is not None:
            self.spacing = np.array(config['data']['img_spacing'], np.float32)
...
def ane_seg_patch_generator(data, config, logger, sliding_window=False, balance_label=True,
    ...
    """
    yield patches of aneurysm segmentation1
    ...
    if any([label_glo_img.shape[i] < patch_size[i] for i in range(3)]):
        logger.log('Subject %s is too small and cannot fit in one patch.' % data['id'])
        return None
    ...
    def _gen_patch(_starts):
        ...
        if patch_cta_img.shape != patch_brain_mask_img.shape or patch_cta_img.shape != patch_label_img.shape:
            logger.log('Different shapes for patch_cta_img, patch_brain_mask_img and patch_label_img: %s, %s, %s'
        ...
        if data_aug:
            ...
            patch_cta_img, patch_brain_mask_img, patch_label_img = np.split(bundle, 3) # 切分 bundle
    ...
    if not sliding_window:
        ...
        logger.log('number of patches generated in %s is %s' % (data['id'], sum_brain_mask_number))
    ...
    # sliding window
        ...
        logger.log('number of patches generated in %s is %s' % (data['id'], len(starts_list)))
