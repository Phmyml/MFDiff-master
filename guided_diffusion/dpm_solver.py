...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None): 
        ...
        if model_type == "noise":
            ...
            print('elif model_type == "x_start":')
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            return (x - alpha_t * output) / sigma_t
        elif model_type == "v":
            print('elif model_type == "v":')
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            return alpha_t * output + sigma_t * x
        elif model_type == "score":
            print('elif model_type == "score":')
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            print('guidance_type == "classifier":')
            ...
            print('guidance_type == "classifier-free":')
            if guidance_scale == 1. or unconditional_condition is None:
                print('guidance_scale == 1. or unconditional_condition is None:')
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        """
        noise = self.noise_prediction_fn(x, t)  # torch.Size([1, 2, 64, 64, 64])
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t)
        x0 = (x - sigma_t * noise[:,0:1,:,:]) / alpha_t
        if self.correcting_x0_fn is not None:
            x0 = self.correcting_x0_fn(x0, t)  # torch.Size([1, 1, 64, 64, 64])
        return x0
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                if self.correcting_xt_fn is not None:
                    x = self.correcting_xt_fn(x, t, step)  # 动态阈值分割 # x.shape[1,1,64,64,64]
