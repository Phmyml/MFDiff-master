...
class GaussianDiffusion:
    ...
    def p_sample_loop_known(
        ...
        if self.dpm_solver:
            ...
            dpm_solver = DPM_Solver(model_fn, noise_schedule, algorithm_type="dpmsolver++",
                            correcting_x0_fn=None, img = img[:, :-1,  ...])  # img = img[:, :-1,  ...]取的是图像数据

            ## Steps in [20, 30] can generate quite good samples.
            
            sample, pred = dpm_solver.sample(
            ...
            cal_out = final["cal"]
            cal_out_softmax = th.nn.Softmax(dim=1)(cal_out)
            cal_out[:,1:,:,:,:] = cal_out_softmax[:,1:,:,:,:] + 0.25 * final["sample"][:,-1,:,:].unsqueeze(1)
        else:
            print('no dpm-solver')
            i = 0
            ...
            if dice_score(final["sample"][:,-1,:,:].unsqueeze(1), final["cal"][:,-1,:,:,:]) < 0.65:
                cal_out[:,1:,:,:,:] = cal_out_softmax[:,1:,:,:,:] + 0.25 * final["sample"][:,-1,:,:,:].unsqueeze(1)
            else:
                cal_out[:,1:,:,:,:] = cal_out_softmax[:,1:,:,:,:]* 0.75 + 0.25 * final["sample"][:,-1,:,:,:].unsqueeze(1)
          

        return final["sample"], x_noisy, img , final["cal"], cal_out, cal_out_softmax
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output, cal= model(x_t, self._scale_timesteps(t), **model_kwargs) # , cal
            if self.model_var_type in [
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
            ]:
            ...
            _, input = th.split(cal, C, dim=1)
            terms["loss_seg"] = self.segloss(input, res, cal)
            # terms["loss_seg"] = mean_flat((res - cal) ** 2)
            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"]
            else:
                terms["loss"] = terms["mse"]
    ...
    def segloss(self, inputs, targets, cal, smooth=1.0,gamma_dice=0.3,gamma_cross=0.3,weight=None):
        y_pred = self.sigmoid(inputs[:,0])
        y_true = targets[:,0].type(th.float32)
        inter = y_pred * y_true
        dice = (2 * (inter.sum(dim=(1, 2, 3))) + smooth) / ((y_true + y_pred).sum(dim=(1, 2, 3)) + smooth)
        loss_dice = ((- dice.log()).clamp_min_(1e-8) ** gamma_dice)
        cross_entropy = th.nn.CrossEntropyLoss(weight=None, ignore_index=-100, reduction='none')
        loss_cross = (cross_entropy(cal, targets[:,0]).clamp_min_(1e-8)) ** gamma_cross
        # if weight is not None:
        #     weight = weight.expand_as(loss_cross)
        #     loss_cross *= weight
        
        loss_cross = loss_cross.mean(dim=(1, 2, 3))
        loss = 0.2 * loss_dice + 0.8 * loss_cross
        return loss
